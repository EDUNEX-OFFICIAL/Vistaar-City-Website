      ...
      <section className="pt-12 pb-8 md:pt-24 md:pb-10 bg-forest-900 relative overflow-hidden">
        <div className="container mx-auto px-4 md:px-8 max-w-7xl relative z-10">
          <div className="text-center max-w-3xl mx-auto mb-8 md:mb-10">
            ...
            <h2 className="font-serif text-2xl sm:text-3xl md:text-4xl font-bold text-white mb-3 sm:mb-4">See Where Your Plot Could Be</h2>
            <p className="text-white/80 text-base sm:text-lg font-light px-2">
          ...
          <div className="mb-4 text-center">
            <p className="text-white/60 text-sm">{activeLayout.description}</p>
            <p className="text-white/90 text-xs font-semibold mt-1">{activeLayout.projectName}</p>
          </div>

          <div className="relative w-full aspect-[4/3] sm:aspect-video md:aspect-[21/9] bg-forest-800 rounded-sm overflow-hidden shadow-2xl border border-white/10">
            <Image
                ...
                {activeLayout.plots.map((plot) => {
                  ...
                  return (
                    <div
                      ...
                      className={`relative aspect-[4/3] min-h-[2.5rem] border-2 border-white/30 backdrop-blur-sm transition-all duration-300 cursor-pointer flex flex-col items-center justify-center gap-0.5 touch-manipulation
                        ${isSold ? "bg-red-500/25 border-red-400/50" : ""}
                        ${isReserved ? "bg-yellow-500/25 border-yellow-400/50" : ""}
                        ${isAvailable ? "bg-forest-500/25 border-forest-300/50 hover:bg-forest-500/45 hover:border-white hover:scale-105" : ""}
                      `}
                      onMouseEnter={() => setHoveredPlot(plot.id)}
                      onMouseLeave={() => setHoveredPlot(null)}
                    >
                      <span className="text-white font-bold text-[10px] md:text-xs">{plot.plotNumber}</span>
                      <span className="text-white/60 text-[8px] md:text-[10px] hidden sm:block">{plot.size}</span>

                      {hoveredPlot === plot.id && (
                        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-40 bg-white rounded-lg shadow-xl p-3 z-20 text-center pointer-events-none">
                          ...
                          <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 w-3 h-3 bg-white transform rotate-45" />
                        </div>
                      )}
                    </div>
                  );
            ...
            <div className="absolute top-2 left-2 sm:top-4 sm:left-4 bg-white/95 rounded-sm px-2 py-1.5 sm:px-3 sm:py-2 shadow-md">
            ...
            <div className="absolute bottom-2 left-2 right-2 sm:bottom-4 sm:left-auto sm:right-4 sm:w-auto bg-white p-2 sm:p-3 md:p-4 rounded-sm shadow-lg flex flex-wrap justify-center gap-2 sm:gap-3 md:gap-4 text-[9px] sm:text-[10px] font-bold text-charcoal uppercase tracking-wider border border-gray-100">
